...
| `import { darken } from "polished"`                      | `shade(color, 10)` / `tint(color, 10)` / `alpha(color, 30)` from `cherry-styled-components`          | Cherry dropped `polished`; the helpers emit native `color-mix()` and accept `var(--token)` inputs.              |
